    ...
    public String loadCustomers(Model model) {
        ...
        try {
            ResponseEntity<Customer[]> responseEntity = restTemplate
                    .exchange("http://localhost:8181/services/customers", HttpMethod.GET, httpEntity, Customer[].class);
            model.addAttribute("customers", responseEntity.getBody());
        } catch (HttpStatusCodeException exception) {
            ResponseEntity responseEntity = ResponseEntity.status(exception.getRawStatusCode())
